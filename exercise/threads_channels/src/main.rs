...
fn main() {
    ...
    let new_vector = vec![2, 5, 1, 0, 4, 3];
    let (tx3,rx3) = channel::unbounded();
    let rx4 = rx3.clone(); // does i really have to do a recevier clone for differents threads
    ...
    let handle_a = thread::spawn(|| {
        for x in rx3 {
            println!("Print from thread 1. Value : {}", x)
        }
    });

    let handle_b = thread::spawn(|| {
        for x in rx4 {
            println!("Print from thread 2. Value : {}", x)
        }
    }); 
